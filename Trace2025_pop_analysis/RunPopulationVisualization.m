...
for p = 1:numel(popNames)
    pop = popNames{p};
    [timeRel, matPop] = collectTrialAlignedMean(SessionRes, TraceNorm, pop, fps, Opts.PlotPadSec);
...
function [timeRel, matOut] = collectTrialAlignedMean(SessionRes, TraceNorm, popName, fps, padSec)
timeRel = [];
matOut = [];
...
minLen = min(cellfun(@numel, traces));
matOut = nan(minLen, numel(traces));
for i = 1:numel(traces)
    matOut(:, i) = traces{i}(1:minLen);
end
timeRel = ((0:minLen-1)' ./ fps) - padSec;
end
