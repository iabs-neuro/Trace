...
Для каждого trial и нейрона:
- считается среднее значение в baseline/CS/Trace/US окнах;

Потом для вектора trial-средних снова применяется `IsResponsive`.

Важно: в текущей версии `MeanAcrossTrials` считается **не по всем нейронам**, а только среди
нейронов, которые были responsive хотя бы в одном trial для соответствующего интервала
(`CS` для CS-оценки, `Trace` для Trace-оценки, `US` для US-оценки).

Это реализует «усреднение по trial» перед статистикой (вместо конкатенации всех фреймов)
с ограничением на trial-responsive pool.
